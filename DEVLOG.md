...
- 混合式关键交互：Home 增加“继续主线”置顶按钮
  ...
  - 逻辑：根据 `GameState.CurrentProgress` 计算下一步 ScreenType（默认会推进到 Semester/Gaokao/Volunteer…）
...
- Home 的“放假/实习小游戏”落地方式（建议）
  - 学期结束：在 Semester 结算后 `NavigateTo(Home, pushToStack=true)`，Home 顶部/中部展示“放假事件/小游戏入口”
